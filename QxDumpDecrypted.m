...
int find_off_cryptid(const char *filePath) {
    NSLog(@"[qxdecrypt] %s filePath: %s", __FUNCTION__, filePath);
    int off_cryptid = 0;
...
-(id) initWithPathToBinary:(NSString *)pathToBinary appName:(NSString *)appName appVersion:(NSString *)appVersion {
	...
	NSLog(@"[qxdecrypt] init: appDirName: %s", lastPartOfAppPath);
...
-(void) makeDirectories:(const char *)encryptedImageFilenameStr {
	...
	NSLog(@"[qxdecrypt] encryptedImagePathStr: %s", encryptedImagePathStr);
	...
	NSLog(@"[qxdecrypt] make_directories making dir: %@", path);
	if(! [fm createDirectoryAtPath:path withIntermediateDirectories:true attributes:nil error:&err]) {
		NSLog(@"[qxdecrypt] WARNING: make_directories failed to make directory %@. Error: %@", path, err);
	}
...
-(BOOL) dumpDecryptedImage:(vm_address_t)imageAddress fileName:(const char *)encryptedImageFilenameStr image:(int)imageNum task:(vm_map_t)targetTask{
    ...
    {
        NSLog(@"[qxdecrypt] Can't read header!");
        exit(1);
    }
	...
	if (header.magic == MH_MAGIC_64) {
		...
		NSLog(@"[qxdecrypt] detected 64bit ARM binary in memory.\n");
		...
		NSLog(@"[qxdecrypt] detected 32bit ARM binary in memory.\n");
	} else {
		NSLog(@"[qxdecrypt] No valid header found!!");
		return false;
	}
	...
	for (i=0; i<image_mh->ncmds; i++) {
		if (lc->cmd == LC_ENCRYPTION_INFO || lc->cmd == LC_ENCRYPTION_INFO_64) {
			...
			if(appFilename == NULL) {
				NSLog(@"[qxdecrypt] There are no / in the filename. This is an error.\n");
				return false;
			}
			appFilename++;
            ...
            NSLog(@"[qxdecrypt] aslr_slide= 0x%x", aslr_slide);

			off_cryptid=find_off_cryptid(encryptedImageFilenameStr);

			NSLog(@"[qxdecrypt] offset to cryptid (%d) found in memory @ %p (from %p). off_cryptid = %u (0x%x)\n", eic->cryptid, &eic->cryptid, image_mh, off_cryptid, off_cryptid);
			//NSLog(@"[qxdecrypt] Found encrypted data at offset %u 0x%08x. image_mh @ %p. cryptedData @ 0x%x. cryptsize = %u (0x%x) bytes.\n", eic->cryptoff, eic->cryptoff, image_mh, (unsigned int)image_mh + eic->cryptoff, eic->cryptsize, eic->cryptsize);
			
			NSLog(@"[qxdecrypt] Dumping: %s", encryptedImageFilenameStr);
			NSLog(@"[qxdecrypt]    Into: %s", self->decryptedAppPathStr);
			fd = open(encryptedImageFilenameStr, O_RDONLY);
			if (fd == -1) {
				NSLog(@"[qxdecrypt] Failed to open %s", encryptedImageFilenameStr);
				return false;
			}
			
			NSLog(@"[qxdecrypt] Reading header");
			n = read(fd, (void *)buffer, sizeof(buffer));
			if (n != sizeof(buffer)) {
				NSLog(@"[qxdecrypt] Warning read only %d of %lu bytes from encrypted file.\n", n, sizeof(buffer));
				return false;
			}

			NSLog(@"[qxdecrypt] Detecting header type\n");
			fh = (struct fat_header *)buffer;
			
			/* Is this a FAT file - we assume the right endianess */
			if (fh->magic == FAT_CIGAM) {
				NSLog(@"[qxdecrypt] Executable is a FAT image - searching for right architecture\n");
				...
				for (i=0; i<swap32(fh->nfat_arch); i++) {
					if ((image_mh->cputype == swap32(arch->cputype)) && (image_mh->cpusubtype == swap32(arch->cpusubtype))) {
						...
						NSLog(@"[qxdecrypt] Correct arch is at offset 0x%x in the file.\n", fileoffs);
						break;
					}
					arch++;
				}
				if (fileoffs == 0) {
					NSLog(@"[qxdecrypt] Could not find correct arch in FAT image\n");
					return false;
				}
			} else if (fh->magic == MH_MAGIC || fh->magic == MH_MAGIC_64) {
				NSLog(@"[qxdecrypt] Executable is a plain MACH-O image, fileoffs = 0\n");
			} else {
				NSLog(@"[qxdecrypt] Executable is of unknown type, fileoffs = 0\n");
				return false;
			}

			NSLog(@"[qxdecrypt] Opening %s for writing.\n", decryptedAppPathStr);
			outfd = open(decryptedAppPathStr, O_RDWR|O_CREAT|O_TRUNC, 0644);
			if (outfd == -1) {
				NSLog(@"[qxdecrypt] Failed opening: ");
				return false;
			}
			...
			//NSLog(@"[qxdecrypt] restsize = %u, n = %u, cryptsize = %u, total = %u", restsize, n, eic->cryptsize, n + eic->cryptsize + restsize);
			lseek(fd, 0, SEEK_SET);
			
			NSLog(@"[qxdecrypt] Copying the not encrypted start of the file (%u bytes)\n", n);
			
			/* first copy all the data before the encrypted data */
			char *buf = (char *)malloc((size_t)n);
			r = read(fd, buf, n);
			if(r != n) {
				NSLog(@"[qxdecrypt] Error reading start of file\n");
				return false;
			}
			r = write(outfd, buf, n);
			if(r != n) {
				NSLog(@"[qxdecrypt] Error writing start of file\n");
				return  false;
			}
			free(buf);
			...
			NSLog(@"[qxdecrypt] Dumping the decrypted data into the file (%u bytes)\n", eic->cryptsize);
			...
			if (r != eic->cryptsize) {
				NSLog(@"[qxdecrypt] Error writing encrypted part of file\n");
				return false;
			}
            free(buf);
			...
			NSLog(@"[qxdecrypt] Copying the not encrypted remainder of the file (%u bytes)\n", restsize);
			lseek(fd, eic->cryptsize, SEEK_CUR);
			buf = (char *)malloc((size_t)restsize);
			r = read(fd, buf, restsize);
			if (r != restsize) {
				NSLog(@"[qxdecrypt] Error reading rest of file, got %u bytes\n", r);
				return false;
			}
			r = write(outfd, buf, restsize);
			if (r != restsize) {
				NSLog(@"[qxdecrypt] Error writing rest of file\n");
				return false;
			}
			free(buf);
			...
			if (off_cryptid) {
				uint32_t zero=0;
				NSLog(@"[qxdecrypt] Setting the LC_ENCRYPTION_INFO->cryptid to 0 at offset 0x%x into file\n", off_cryptid);
				if (lseek(outfd, off_cryptid, SEEK_SET) == off_cryptid) {
					if(write(outfd, &zero, 4) != 4) {
						NSLog(@"[qxdecrypt] Error writing cryptid value!!\n");
						// Not a fatal error, just warn
					}
				} else {
					NSLog(@"[qxdecrypt] Failed to seek to cryptid offset!!");
...
-(void) dumpDecrypted:(pid_t)pid {
    ...
    {
        NSLog(@"[qxdecrypt] Can't execute task_for_pid! Do you have the right permissions/entitlements?\n");
        exit(1);
    }
    ...
    NSLog(@"[qxdecrypt] There are %d images mapped.", numberOfImages);

    for (int i = 0; i < numberOfImages; i++) {
        NSLog(@"[qxdecrypt] image %d", i);
        ...
        NSLog(@"[qxdecrypt] Comparing %s to %s", imageName, appPath);

        if(strstr(imageName, appPath) != NULL) {
			NSLog(@"[qxdecrypt] Dumping image %d: %s", i, imageName);
...
-(void) createIPAFile:(pid_t)pid {
	...
	NSLog(@"[qxdecrypt] ======== START FILE COPY - IGNORE ANY SANDBOX WARNINGS ========");
	NSLog(@"[qxdecrypt] IPAFile: %@", IPAFile);
	NSLog(@"[qxdecrypt] appDir: %@", appDir);
	NSLog(@"[qxdecrypt] appCopyDir: %@", appCopyDir);
	NSLog(@"[qxdecrypt] zipDir: %@", zipDir);
	
	[fm copyItemAtPath:appDir toPath:appCopyDir error:&err];
	NSLog(@"[qxdecrypt] ======== END OF FILE COPY ========");
    // sleep(1);
    // exit(1);
	// Replace encrypted binaries with decrypted versions
	NSLog(@"[qxdecrypt] ======== START DECRYPTION PROCESS ========");
	[self dumpDecrypted:pid];
	NSLog(@"[qxdecrypt] ======== DECRYPTION COMPLETE  ========");

	// ZIP it up
	NSLog(@"[qxdecrypt] ======== STARTING ZIP ========");
	NSLog(@"[qxdecrypt] IPA file: %@", IPAFile);
	NSLog(@"[qxdecrypt] ZIP dir: %@", zipDir);
	...
	@try {
		...
		NSLog(@"[qxdecrypt] ========  ZIP operation complete: %s ========", (success)?"success":"failed");
	}
	@catch(NSException *e) {
		NSLog(@"[qxdecrypt] BAAAAAAAARF during ZIP operation!!! , %@", e);
	}
	...
	NSLog(@"[qxdecrypt] ======== Wrote %@ ========", [self IPAPath]);
	return;
}
