...
void decryptApp(NSDictionary *app) {
    ...
    NSLog(@"[qxdecrypt] spawning thread to do decryption in background...");

    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        NSLog(@"[qxdecrypt] inside decryption thread.");
        ...
        if (pid == -1) {
            dispatch_async(dispatch_get_main_queue(), ^{
                ...
                NSLog(@"[qxdecrypt] failed to get pid for binary name: %@", binaryName);
...
void bfinject_rocknroll(pid_t pid, NSString *appName, NSString *version) {
    NSLog(@"[qxdecrypt] Spawning thread to do decryption in the background...");
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        NSLog(@"[qxdecrypt] fullPathStr: %s", fullPathStr);
        ...
        if(!dd) {
            NSLog(@"[qxdecrypt] ERROR: failed to get DumpDecrypted instance");
            return;
        }

        NSLog(@"[qxdecrypt] Full path to app: %s   ///   IPA File: %@", fullPathStr, [dd IPAPath]);
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            alertController = [UIAlertController
                alertControllerWithTitle:@"Descifrando"
                message:@"Por favor espere, esto tomara unos segundos..."
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            doneController = [UIAlertController alertControllerWithTitle:@"Descifrado completado!" message:[NSString stringWithFormat:@"Archivo IPA  guardado en:\n%@", [dd IPAPath]] preferredStyle:UIAlertControllerStyleAlert];
            ...
            if ([[UIApplication sharedApplication] canOpenURL:[NSURL URLWithString:@"filza://"]]) {
                UIAlertAction *openAction = [UIAlertAction actionWithTitle:@"Mostrar En Filza" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                    [kw removeFromSuperview];
                    kw.hidden = YES;
        ...
        NSLog(@"[qxdecrypt] Over and out.");
        while(1)
            sleep(9999999);
    }); // dispatch in background
    
    NSLog(@"[qxdecrypt] All done, exiting constructor.");
}
...
NSString *docPath(void) {
    NSError * error = nil;
    [[NSFileManager defaultManager] createDirectoryAtPath:@"/var/mobile/Library/QxDecrypt/decrypted" withIntermediateDirectories:YES attributes:nil error:&error];
    if (error != nil) {
        NSLog(@"[trolldecrypt] error creating directory: %@", error);
    }

    return @"/var/mobile/Library/QxDecrypt/decrypted";
}
...
void decryptAppWithPID(pid_t pid) {
    ...
    if (message) {
        dispatch_async(dispatch_get_main_queue(), ^{
            [alertController dismissViewControllerAnimated:NO completion:nil];
            NSLog(@"[qxdecrypt] failed to get bundleid for pid: %d", pid);
    ...
    NSLog(@"[qxdecrypt] appInfo: %@", appInfo);

    dispatch_async(dispatch_get_main_queue(), ^{
        [alertController dismissViewControllerAnimated:NO completion:nil];
        UIAlertController *alert = [UIAlertController alertControllerWithTitle:@"Descifrar" message:[NSString stringWithFormat:@"Decrypt %@?", appInfo[@"name"]] preferredStyle:UIAlertControllerStyleAlert];
...
//API Working on it later 
void github_fetchLatedVersion(NSString *repo, void (^completionHandler)(NSString *latestVersion)) {
    NSString *urlString = [NSString stringWithFormat:@"https://github.com/QxAngel/QxDecrypt/releases/latest", repo];
...
void fetchLatestQxDecryptVersion(void (^completionHandler)(NSString *version)) {
    github_fetchLatedVersion(@"QxAngel/QxDecrypt", completionHandler);
}

NSString *qxDecryptVersion(void) {
